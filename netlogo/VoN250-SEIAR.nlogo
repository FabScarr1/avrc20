...
;; in una classe tutti sono in contatto con tutti
...
create-turtles number-of-nodes [ set color green
  ...
  ;; link interclasse con una certa probabilita'
  ask turtles [
    ask turtles with [ who > [ who ] of myself ] [
      if random 100 < 4 [
        create-link-with myself
...
to go
  if all? turtles [not infected? and not exposed? and not asymptomatic?]
    [ stop ]
  ask turtles
  [
    if infected? [
