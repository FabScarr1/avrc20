...
;; all'interno della classe tutti entrano in contatto con tutti
...
create-turtles number-of-nodes [ set color green
  ...
  ;; link fra tutti con una certa probabilita'
  ask turtles [
    ask turtles with [ who > [ who ] of myself ] [
      if random 100 < 4 [
        create-link-with myself
...
to do-virus-checks
  ...
  ask turtles with [asymptomatic? and virus-check-timer = 0]
  [
     become-resistant
  ]
end
...
Each time step (tick), each infected node (colored red) attempts to infect all of its neighbors.  Susceptible neighbors (colored green) will become exposed (yellow) with a probability given by the virus-spread-chance slider after
v-E-check-freq tick an exposed node becomes asymptomatic with probability (prob-asymptomatic) or infected with probability (1 - prob-asymptomatic).
After virus-check-frequency an infected or asymptomatic node becomes resistant (gray)  
and cannot be infected.  

when a node is detectd as infected it is locked for v-L-check-freq ticks 
after that delay it becomes resistant.
